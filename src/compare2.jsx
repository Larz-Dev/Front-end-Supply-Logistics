...
const Compare = () => {
  ...
  let [noHay, setNohay] = useState(0);
  let [siHay, setSihay] = useState(0);
  let [errorHay, setErrorhay] = useState(0);
  ...
  const TranscribirExcel = async (sheet) => {
    ...
    if (
      ...
      setFechaSupplyInicio(cutJson[cutJson.length - 1][1]);
      setFechaSupplyFin(cutJson[0][1]);

      setHoraSupplyInicio(cutJson[cutJson.length - 1][1]);
      setHoraSupplyFin(cutJson[0][1]);
  ...
  const generateThirdTable = () => {
    ...
    dataExcel2.forEach((row) => {
      placaStatus[row[5]] = row[48]; // Almacenar el estado de cada placa
    });
   
    
    // Extender los registros de dataExcel1 y intercambiar la primera y última columna
    const thirdTable = dataExcel2
      ...
      .map((row) => {
        ...
        if (
          ...
          if (row[48].includes("Posible error de placa")) {
            console.log(row[48])
            errorHay += 1;
          }
        ...
        <div className="row">
          <div className="col-6 ">
            ...
            >
              <table className="table table-striped table-hover">
                <thead>
                  <tr>
                    <th className="bg-supply border-0 text-white">Fecha</th>
                    <th className="bg-supply border-0 text-white">Placa</th>
                    <th className="bg-supply border-0 text-white">
                      Transportadora
                    </th>
                    <th className="bg-supply border-0 text-white">Area</th>
                    <th className="bg-supply border-0 text-white">
                      Tipo vehiculo
                    </th>
                    <th className="bg-supply border-0 text-white">
                    ...
                    <th className="bg-supply border-0 text-white">Valor</th>
                    <th className="bg-supply border-0 text-white">Pago</th>
                    <th className="bg-supply border-0 text-white">
                      Referencia
                    </th>
                  </tr>
                ...
                <tbody>
                  {filteredRows.map((row, index) => (
                    <tr key={index}>
                      <td>
                        {ExcelDateToJSDate(StringDateToExcelDate(row[1])) +
                          " " +
                          ExcelDateToJSTime(StringDateToExcelDate(row[1]))}
                      </td>
                      <td className="fw-bold">{row[3]}</td>
                      <td className={row[5] == null ? "bg-danger" : ""}>
                        {row[5]}
                      </td>
                      <td>{row[2]}</td>
                      <td>{""}</td>
          ...
          <div className="col-6 ">
            ...
            >
              <table className="table  table-striped table-hover">
                <thead>
                  <tr>
                    <th className="bg-supply border-0 text-white">Estado</th>
                    <th className="bg-supply border-0 text-white">Placa</th>
                    <th className="bg-supply border-0 text-white">Fecha</th>
                    <th className="bg-supply border-0 text-white">
                      Observación
                ...
                <tbody>
                  {filteredRows2.map((row, index) => (
                    <tr key={index}>
                      <td
                        className={
                          row[48] === "No está"
                            ...
                            : row[48] === "Predicción"
                            ? "bg-prediccion"
                            : row[48].includes("Posible error de placa")
                            ? "bg-revisar"
                            : ""
                        }
                      >
                      ...
                      <td className="fw-bold">{row[5]}</td>
                      <td>
                        {ExcelDateToJSDate(row[44]) +
                          " " +
                          ExcelDateToJSTime(row[44])}
                      </td>
                      <td>{row[47]}</td>
                      <td>{row[1]}</td>
                    </tr>
          ...
          <div className="col-12 mt-5">
            ...
            {thirdTableData.length > 0 && (
              <div>
                <div className="fs-4 d-grid">
                  <div className="border p-3   bg-esta  fw-bold ">
                    Sí están : {siHay}
                  </div>
                  <div className="border p-3  bg-noesta  fw-bold ">
                    No están : {noHay}
                  </div>
                  <div className="border p-3 bg-revisar  fw-bold ">
                    Errores posibles : {errorHay}
                  </div>
                </div>
                <br />
              </div>
            ...
            >
              <table className="table table-striped table-hover">
                <thead>
                  <tr>
                    {dataExcel1.length > 0 && dataExcel2.length > 0 ? (
                      Array.from({ length: 49 }, (_, index) => (
                        <th
                          className="bg-supply border-0 text-white"
                          key={index}
                        >
                          {" "}
                          {columnNames[index]}
                      ...
                      <th>
                        Subir los documentos requeridos para mostrar la tabla
                      </th>
                    )}
                  </tr>
                </thead>
                        ...
                        <td
                          ...
                          className={
                            cellIndex === 0 && cell === "No está"
                              ...
                              ? "bg-esta"
                              : cellIndex === 0 && cell === "Predicción"
                              ? "bg-prediccion"
                              : cellIndex === 0 &&
                                cell.includes("Posible error de placa")
                              ? "bg-revisar"
                              : cellIndex === 6
                              ? "fw-bold"
                              : ""
